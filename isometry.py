...
class Camera():
    ...
    def render_tile(self, img, params):
        ...
        for i in range(stacks*int(4/scale)):
            ...
            tile_surface.blit(layer, (0,(stacks+offset)*int(4/scale)-i))

        w, h = tile_surface.get_size()
        tile_surface = pg.transform.scale(tile_surface,(w*2,h*2))

        return [tile_surface, (stacks+offset)*4]
    ...
    def draw_map(self, trains, font):
        ...
        screen = pg.Surface([i/min(1,self.scale) for i in self.camera_size],pg.SRCALPHA)
        screen.fill((255,254,249))
        ...
        for dy in range(bm_pos[1]-a,bm_pos[1]+a):
            ...
            for dx in range(bm_pos[0]-a,bm_pos[0]+a):
                ...
                if f"{dx}:{dy}" in self.rail_nodes:
                    for track in self.rail_nodes[f"{dx}:{dy}"][0]+self.rail_nodes[f"{dx}:{dy}"][1]:
                        tracks.append(track)
                ...
                if self.debug:
                    ...
                    pg.draw.lines(screen, (247,246,242),True,(p1,p2,p3,p4),2)
        ...
        for dy in range(bm_pos[1]-a,bm_pos[1]+a):
            ...
            pg.draw.line(screen, (247,246,242),p1,p2,4)
        ...
        for train in trains:
            if (21*256*(chunk_pos[0]-1) < train.pos[0] < 21*256*(chunk_pos[0]+2) and 
                ...
                train.type != None):
                #рисуем
                angle = train.angle//1 # угол//1 = инд. в массиве
                ...
                pg.draw.circle(screen,(255,0,0),center_pos,4)
